...
class ErrorReportAPITest(HDSAPITestBase):
    ...
    def test_extract_errors(self):
        ...
        compare = {
            ...
            "timestamp": ErrorReportSerializer.extract_timestamp(
                errdict[serv_str], key="ts"
            ),
            "handled": False,
        }
        ...
        self.assertTrue(
            all([primary.timestamp <= e.timestamp for e in excs])
        )  # primary is first
    ...
    def test_create_emu_report(self):
        ...
        resp = self.client.post(self.error_url, data, format="json")
    ...
    def test_get_emu_get_non_emu(self):
        ...
        self.client.post(self.error_url, self.data, format="json")
    ...
    def test_catch_extract_exc_errors(self, mock_logger):
        ...
        data["data"]["sysmon_report"]["sysmon.0"]["errors"] = {"traychg_0": {}}
        expected_error = ValueError.__name__
        counter = ASYNC_ERROR_COUNTER.labels(
            "_extract_exception_data", expected_error, FAILED_SPLIT_MSG
        )

        r = self.client.post(self.error_url, data, format="json")
    ...
    def test_query_errorreport(self):
        ...
        harv_dict = {"harv_ids": ",".join(map(str, [harv_id]))}
        resp = self.client.get(f"{self.error_url}?{urlencode(harv_dict)}")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(
            resp.json()["data"]["results"][0]["harvester"]["harv_id"], harv_id
        )
        ...
        harv_dict.update({"harv_ids": ",".join(map(str, [harvester.harv_id]))})
        resp = self.client.get(f"{self.error_url}?{urlencode(harv_dict)}")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        ...
        ranch_dict.update({"locations": ",".join(map(str, [location.ranch]))})
        resp = self.client.get(f"{self.error_url}?{urlencode(harv_dict)}")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json()["data"]["results"]), 0)
        ...
        # query start_time
        start_dict = {
            "start_time": DTimeFormatter.localize_to_tz(self.data["timestamp"])
        }
        resp = self.client.get(f"{self.error_url}?{urlencode(start_dict)}")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json()["data"]["results"]), 1)

        # query start_time add 1
        start_dict = {
            "start_time": DTimeFormatter.localize_to_tz(
        ...
        fruit = self.test_objects["fruit"]
        fruit_dict = {"fruits": ",".join(map(str, [fruit.name]))}
        ...
        fruit_dict.update({"fruits": ",".join(map(str, [fruit.name]))})
        resp = self.client.get(f"{self.error_url}?{urlencode(fruit_dict)}")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json()["data"]["results"]), 0)
        ...
        codes_dict = {"codes": ",".join(map(str, [code]))}
        ...
        self.post_error_report(load=False)
        # query traceback
        trace_dict = {"traceback": "findme"}
        resp = self.client.get(f"{self.error_url}?{urlencode(trace_dict)}")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json()["data"]["results"]), 1)
        ...
        code_dict = {"codes": ",".join(map(str, set(codes))), "primary": True}
        resp = self.client.get(f"{self.error_url}?{urlencode(code_dict)}")
        self.assertEqual(resp.json()["data"]["count"], 3)

        # primary exception codes with primary flag set to false
        # e.g 0*,0
        code_dict = {"codes": ",".join(map(str, set(codes))), "primary": False}
        resp = self.client.get(f"{self.error_url}?{urlencode(code_dict)}")
        self.assertEqual(resp.json()["data"]["count"], 3)
    ...
    def test_create_report_withno_exceptions(self):
        ...
        primary_dict = {"primary": True}
        resp = self.client.get(f"{self.error_url}?{urlencode(primary_dict)}")
        self.assertEqual(resp.json()["data"]["count"], 0)

        # query for primary flag set to false
        primary_dict = {"primary": False}
        resp = self.client.get(f"{self.error_url}?{urlencode(primary_dict)}")
        self.assertEqual(resp.json()["data"]["count"], 0)

        # query for harvester only
        harv_only = {"is_emulator": False}
        resp = self.client.get(f"{self.error_url}?{urlencode(harv_only)}")
        self.assertEqual(resp.json()["data"]["count"], 1)

        # query for harv_ids
        harvids_dict = {"harv_ids": "".join(map(str, [11]))}
        resp = self.client.get(f"{self.error_url}?{urlencode(harvids_dict)}")
        self.assertEqual(resp.json()["data"]["count"], 1)

        # query for fruits
        fruit_dict = {"fruits": "".join(map(str, ["strawberry"]))}
        resp = self.client.get(f"{self.error_url}?{urlencode(fruit_dict)}")
        self.assertEqual(resp.json()["data"]["count"], 1)

        fruit_dict = {"fruits": "".join(map(str, ["apple"]))}
        resp = self.client.get(f"{self.error_url}?{urlencode(fruit_dict)}")
        self.assertEqual(resp.json()["data"]["count"], 0)
    ...
    def test_filter_for_time_of_day(self):
        ...
        resp = self.client.get(f"{self.error_url}?{urlencode(filter_dict)}")
        self.assertEqual(resp.json()["data"]["count"], 1)
