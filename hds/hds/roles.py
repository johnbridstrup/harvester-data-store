...
class RoleChoices(models.TextChoices):
    ...
    MANAGER = (
        "manager"  # Can access all resources, create users, deploy code, etc...
    )
...
def is_manager(request, view):
    return any(
        [x(request, view) for x in [is_admin, is_role(RoleChoices.MANAGER)]]
    )


def is_dev(request, view):
    return any(
        [x(request, view) for x in [is_manager, is_role(RoleChoices.DEVELOPER)]]
    )


def is_support(request, view):
    return any(
        [x(request, view) for x in [is_dev, is_role(RoleChoices.SUPPORT)]]
    )
