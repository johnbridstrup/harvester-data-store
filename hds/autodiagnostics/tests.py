...
class AutodiagnosticsApiTestCase(HDSAPITestBase):
    ...
    def test_extract_values(self):
        ...
        self.assertEqual(
            int(gripper.serial_number), int(self.ad_data["data"]["serial_no"])
        )
    ...
    def test_common_filters(self):
        ...
        r1_before = self.client.get(
            f"{self.ad_run_url}?created_before={make_naive(t1)}&tz=utc"
        )
        self.assertEqual(r1_before.json()["data"]["count"], 0)
        r1_after = self.client.get(
            f"{self.ad_run_url}?created_after={make_naive(t1)}&tz=utc"
        )
        self.assertEqual(r1_after.json()["data"]["count"], 2)

        r2_before = self.client.get(
            f"{self.ad_run_url}?created_before={make_naive(t2)}&tz=utc"
        )
        self.assertEqual(r2_before.json()["data"]["count"], 1)
        r2_after = self.client.get(
            f"{self.ad_run_url}?created_after={make_naive(t2)}&tz=utc"
        )
        self.assertEqual(r2_after.json()["data"]["count"], 1)

        r3_before = self.client.get(
            f"{self.ad_run_url}?created_before={make_naive(t3)}&tz=utc"
        )
        self.assertEqual(r3_before.json()["data"]["count"], 2)
        r3_after = self.client.get(
            f"{self.ad_run_url}?created_after={make_naive(t3)}&tz=utc"
        )
        ...
        r_tz_before = self.client.get(
            f"{self.ad_run_url}?created_before={t2_othertz}&tz={tz}"
        )
        self.assertEqual(r_tz_before.json()["data"]["count"], 1)
        r_tz_after = self.client.get(
            f"{self.ad_run_url}?created_after={t2_othertz}&tz={tz}"
        )
    ...
    def test_report_filters(self):
        ...
        self.ad_data["timestamp"] = t1 + 0.00001  # shift for gte and lte
        dt1 = timezone.datetime.fromtimestamp(t1)  # This is UTC
        self.post_autodiag_report(load=False)

        t2 = time.time()
        dt2 = timezone.datetime.fromtimestamp(t2)
        self.ad_data["timestamp"] = t2 + 0.00001
        self.post_autodiag_report(load=False)
        ...
        # Fruit and ranch
        r_straw = self.client.get(
            f"{self.ad_url}?fruits={self.test_objects['fruit'].name}"
        )
        ...
        r_ranch = self.client.get(
            f"{self.ad_url}?locations={self.test_objects['location'].ranch}"
        )
    ...
    def test_filter_datetime_range(self):
        ...
        r2 = self.client.get(
            f"{self.ad_run_url}?datetime_range={start_dt_str},{end_dt_str}"
        )
