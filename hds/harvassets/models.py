...
class HarvesterAssetReport(PickSessionModelMixin, ReportBase):
    assets = models.ManyToManyField(
        "HarvesterAsset", related_name="linked_asset_reports"
    )
...
class HarvesterAssetType(CommonInfo):
    ...
    def get_or_create(asset_type, user):
        try:
            ...
            asset_type_obj = HarvesterAssetType.objects.create(
                creator=user, name=asset_type
            )
...
class HarvesterAsset(CommonInfo):
    ...
    asset = models.ForeignKey(
        HarvesterAssetType, on_delete=models.CASCADE, related_name="assets"
    )
    ...
    @classmethod
    def update_or_create_and_get(
        cls, asset_type_obj, harv, index, serial_number, user, version=None
    ):
        ...
        exist_at_index = cls.objects.filter(
            asset=asset_type_obj, harvester=harv, index=index
        )
        ...
        try:
            asset_obj = HarvesterAsset.objects.get(
                asset=asset_type_obj, serial_number=serial_number
            )
