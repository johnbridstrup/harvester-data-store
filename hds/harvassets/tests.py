...
class HarvesterAssetsTestCase(HDSAPITestBase):
    ...
    def test_filter_reports_for_asset(self):
        ...
        report4 = self.create_report(
            self.asset("A", serial_number=11), self.asset("B", serial_number=21)
        )
    ...
    def test_extraction_multi_type(self):
        ...
        report = self.create_report(*[self.asset(asset=i) for i in types])
    ...
    def test_get_assets_params(self):
        ...
        report = self.create_report(
            *[
                self.asset(asset=i, serial_number=j, index=j)
                for i, j in zip(types, range(1, 4))
            ]
        )
        self.client.post(self.assetrep_url, report, format="json")
        ...
        new_report = self.create_report(
            *[
                self.asset(asset=i, serial_number=j + 3, index=j)
                for i, j in zip(types, range(1, 4))
            ]
        )
    ...
    def test_compile_report(self):
        ...
        for asset in self.asset_data["data"]:
            ...
            self.assertTrue(
                [
                    str(asset["asset-tag"]) == d["serial number"]
                    for d in report[harv_key][asset["asset"]]
                ]
            )
            self.assertTrue(
                [
                    int(asset["index"]) == d["index"]
                    for d in report[harv_key][asset["asset"]]
                ]
            )
