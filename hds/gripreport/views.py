...
from .serializers.candidateserializers import (
    ...
    CandidateFlattenedListSerializer,
)
from .serializers.gripserializers import (
    GripFullSerializer,
    GripMinimalSerializer,
    GripFlattenedListSerializer,
)
...
class GripReportView(ReportModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        payload_type = request.headers.get("X-Payload-Type")
        if payload_type and payload_type == "generic/json":
            ...
            event = request.data
            download_gripreport.apply_async(
                args=(event, request.user.id), link=extract_grip_report.s()
            )
