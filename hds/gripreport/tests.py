...
class GripReportTestCase(HDSAPITestBase):
    ...
    def test_cand_filters(self):
        ...
        resp = self.client.get(
            url,
            {
                "start_time": good_start.strftime("%Y-%m-%dT%H:%M:%S%z"),
                "end_time": good_end.strftime("%Y-%m-%dT%H:%M:%S%z"),
            },
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json()["data"]["results"]), 3)
        ...
        resp = self.client.get(
            url, {"start_time": bad_start.strftime("%Y-%m-%dT%H:%M:%S%z")}
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json()["data"]["results"]), 0)
        ...
        # Harvester Filter
        resp = self.client.get(
            url, {"harv_ids": self.test_objects["harvester"].harv_id}
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json()["data"]["results"]), 3)
    ...
    def test_grip_extraction(self):
        ...
        for cand in cand_list:
            if (
                cand["cand_id"] == exp_grip["cand_id"]
                and cand["robot_id"] == exp_grip["robot_id"]
            ):
                exp_cand = cand
                break

        cand_obj = Candidate.objects.get(
            cand_id=exp_cand["cand_id"], robot_id=exp_cand["robot_id"]
        )
